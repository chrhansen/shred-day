describe('CSV Export Page', () => {
  ...
  it('should allow selecting seasons, customizing columns, and initiating download', () => {
    ...
    // Uncheck 'Labels' (ID 'labels') - These are standard input checkboxes
    cy.get('#column-selector-content input[type="checkbox"][id="labels"]').uncheck({ force: true });
    ...
    cy.wait('@downloadCsv').then((interception) => {
      ...
      const labelsColumn = requestPayload.columns.find((col: {id: string}) => col.id === 'labels');
      expect(labelsColumn, 'Labels column should not be found in payload').to.be.undefined;

...
Plan for replacing the single-value `activity` field with user-defined, multi-select tags that can be created and removed inline on the Ski Day form.
...
- **Serializers.** Extend day serializers (`DayEntrySerializer`, detailed serializer used by `SkiDayDetail`, etc.) to include a `tags` array (each item containing `id` and `name`) so the UI can display multiple entries.
- **Routes.** Add `resources :tags, only: [:index, :create, :destroy]` under the `/api/v1` namespace.

## 4. Frontend Updates
- **Types & services.** Introduce a `Tag` TypeScript type plus a `tagService` mirroring the ski service (list/create/delete). Update day-related types so `activity` is replaced with `tags: Tag[]` and `tag_ids` in mutation payloads.
- **useLogDay hook.** Fetch the user’s tags via TanStack Query, manage a `selectedTagIds` state array, and expose helpers to toggle tags, create a tag (call `tagService.create`, append to cache, auto-select), and delete a tag (only if unused). Remove the `selectedActivity` requirement from form validation.
- **Form component.** Replace `ActivitySelection` with a new `TagSelection` component:
  - Shows chips for every available tag with multi-select behavior (like skis).
  - Provides inline add UI identical to skis for creating tags.
  - Offers a delete affordance (e.g., small `X` on each tag) that calls the backend delete endpoint and displays validation errors if the tag is still attached to any day.
  - Allows zero selections.
- **Day views.** Update `SkiDayDetail`, `SkiDayItem`, `DayCard`, and any other components to render the new tags list (chips or comma-separated text). Hide the section if no tags exist.
- **Clipboard/upload flows.** Update `useLogDay` mutations and any other API consumers (imports, CSV export, etc.) to pass `tag_ids` and read `tags` from responses.
...
- **Frontend tests.** Update service mocks, add component tests for `TagSelection`, and extend hook/page tests to cover adding/removing tags and submitting days with zero tags.
- **Manual QA.** Verify:
  1. Existing days retain their previous “activity” as tags post-migration.
  2. Creating/editing days allows 0+ tags.
  3. New users start with the three default tags.
  4. Attempting to delete an in-use tag surfaces the expected error in the UI.
